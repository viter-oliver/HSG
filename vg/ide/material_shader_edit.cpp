...
void material_shader_edit::draw_shader()
{
	...
	if (pshd_sel&&ImGui::BeginPopupContextWindow())
	{
		...
		{
			auto& it_del = g_vg_shader_list.find(sd_key_name);
			g_vg_shader_list.erase(it_del);
			pshd_sel = nullptr;
			sd_key_name = "";
		}
		ImGui::EndPopup();
...
COMPAIR_SHADER_END:
			...
			g_vg_shader_list[shd_name_str] = pshd;
