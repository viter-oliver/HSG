...
#endif
#include "cp_shader.h"
#include "cp_material.h"
#include "cp_primitive_object.h"
#include "cp_model.h"
#include "cp_bind.h"
#include "cp_state_manager.h"
#include "cp_feedback.h"
#include "cp_playlist_group.h"
...
using namespace vg;
void afb_load::init_ui_component_by_mgo(control_common_def *&ptar, msgpack::v2::object &obj)
{
...
#define TIME_CHECK(x)
	...
	unpac.next(oh);
	auto obj_w = oh.get();
	control_common_def::screenw = obj_w.via.array.ptr[en_screen_w].as<float>();
	control_common_def::screenh = obj_w.via.array.ptr[en_screen_h].as<float>();
	...
	auto obj_2_prp_pos = [this](msgpack::v2::object &okey, prop_ele_position &prp_epos)
	{
		...
		int ii = rsz - 1;
		control_common_def *pcontrol = _pj;
		for (; ii > 1; ii--)
		{
