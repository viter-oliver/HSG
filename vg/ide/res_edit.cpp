...
{
	ImGui::SliderFloat("Width of screen", &control_common_def::screenw,100.,2000.);
	ImGui::SliderFloat("Height of screen", &control_common_def::screenh, 100., 2000.);

	int isize=g_vpacking_texture.size();
     ImGui::Combo( "Texture group list", &_cur_texture_id_index, get_texture_group_name, &g_vpacking_texture, isize );
	...
	static char texture_data_name_str[FILE_NAME_LEN] = "";
     ...
     {
          auto sd_new_res_txtlist=make_shared<packing_texture>();
          packing_texture& retxt = *sd_new_res_txtlist;
          ...
          {
               g_vpacking_texture.emplace_back( sd_new_res_txtlist);
          } else {
...
{
	if (g_vpacking_texture.size()>0)
	{
		ImGui::Text("texture data file:");
		ImGui::SameLine();
          auto& res_gp = *g_vpacking_texture[ _cur_texture_id_index ];
