...
shared_ptr<vg_texture> _ptexture{ nullptr };
string _key_name;
...
void texture_edit::draw_texture_list()
{
	...
	{
		for (auto& mtt_ut : g_dic_indep_texture)
		{
	...
	if (_ptexture&&ImGui::BeginPopupContextWindow())
	{
		...
		{
			auto& item_del = g_dic_indep_texture.find(_key_name);
			g_dic_indep_texture.erase(item_del);
			_ptexture = nullptr;
			_key_name = "";
		}
		ImGui::EndPopup();
