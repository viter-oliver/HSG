...
void slider_path_picker::view()
{
	...
	auto vtx_sz = g_vpacking_texture.size();
	if (vtx_sz == 0)
	{
		return;
	}
	...
	if (_cur_tacks_file_name.empty())
	{
		...
		{
			auto ifl = g_dic_file_chunk.find(track_file_name);
			if (ifl == g_dic_file_chunk.end())
			{
				track_file_name[0] = '\0';
				return;
			}
	...
	if (ImGui::Button("Save to track file"))
	{
		...
		if (_cur_tacks_file_name.empty())
		{
			if (strlen(track_file_name) > 0)
			{
				...
				auto file_key_name = find_a_key_from_mp(g_dic_file_chunk, key_tmp);
				auto ps_file = make_shared<vg_file>(file_sz);
				...
				*file_head = alen;
				...
				g_dic_file_chunk[file_key_name] = ps_file;
		...
		{
			auto ifl = g_dic_file_chunk.find(_cur_tacks_file_name);
	...
	ImGui::Combo("Group id of track obj", &_img_id.x, get_texture_group_name, &g_vpacking_texture, igsize);
	auto &res_gp = *g_vpacking_texture[_img_id.x];
