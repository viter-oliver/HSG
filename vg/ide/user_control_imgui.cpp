...
bool IconTreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags,
  ...
  ImRect frame_bb =
      ImRect(window->DC.CursorPos,
             ImVec2(window->Pos.x + ImGui::GetContentRegionMax().x,
  ...
  bool hovered, held,
      pressed =
          ImGui::ButtonBehavior(interact_bb, id, &hovered, &held, button_flags);
  if (!(flags & ImGuiTreeNodeFlags_Leaf)) {
    bool toggled = false;
    if (pressed) {
  ...
  struct nestfun {
    ...
    bool will_draw;
    nestfun(ImVec2 &bpos, const char *icname, float &txtxof,
            const char *icon_postfix)
        : _basepos(bpos), _iconname(icname), _txtxoffset(txtxof),
  ...
  if (display_frame) {
    // Framed type
    ImGui::RenderFrame(frame_bb.Min, frame_bb.Max, col, true,
                       style.FrameRounding);
    ImGui::RenderNavHighlight(frame_bb, id, ImGuiNavHighlightFlags_TypeThin);
    ImGui::RenderArrow(frame_bb.Min + ImVec2(padding.x, text_base_offset_y),
                       is_open ? ImGuiDir_Down : ImGuiDir_Right, 1.0f);
    tmpfun();
    ImGui::RenderTextClipped(text_pos, frame_bb.Max, label, label_end,
                             &label_size);
  } else {
    ...
    if (flags & ImGuiTreeNodeFlags_Bullet)
      ImGui::RenderBullet(frame_bb.Min +
