...
void pack_ui_component_data(control_common_def &tar,
#ifdef _DX5_COMPRESS
...
void hsb_output::output_hsb(const char *afb_file)
{
	ofstream fout;
	...
	pk.pack_float(control_common_def::screenw); // en_screen_w
	pk.pack_float(control_common_def::screenh); // en_screen_h
	// vfont_face_name& ft_nm_list = font_manager::get_font_name_list();
	auto &dic_ft = font_manager::get_dic_fonts();
	...
	for (auto &ft_item : dic_ft)
	{
		...
		get_file_data(font_name_path, [&](char *file_buff, unsigned int data_len)
										 {
	...
	int v_sz = g_vpacking_texture.size();
	fout_put.begin_enum("enum_txt_res_group");
	stringstream stm_ix;
	for (int ix = 0; ix < v_sz; ++ix)
	{
		fout_put.push_enum_name(g_vpacking_texture[ix]->texture_pack_file);
	}
	fout_put.end_enum();
	for (auto &res_item : g_vpacking_texture)
	{
	...
	pk.pack_array(g_dic_indep_texture.size()); // en_mtextures_res
	for (auto &txt_unit : g_dic_indep_texture)
	{
	...
	pk.pack_array(g_dic_file_chunk.size()); // en_mfiles_res
	for (auto &file_unit : g_dic_file_chunk)
	{
	...
	pk.pack_array(g_cp_shader_list.size()); // en_shaders_res

	for (auto &shd_ut : g_cp_shader_list)
	{
		pk.pack_array(3);
