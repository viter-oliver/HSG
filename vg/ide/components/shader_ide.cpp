...
void create_item(shader::mp_sd_shder_base &shader_list) {
  ...
  ImGui::SameLine();
  if (ImGui::Button("Load vertex shader file...")) {
    ...
    if (GetOpenFileName(&ofn)) {
      string sd_dir_path = g_cureent_directory + shaders_fold;
...
void select_item(shader::sd_shader_ide &sd_shd_i) {
  strcpy(vs_code, sd_shd_i->_vs_code.c_str());
  vs_code[sd_shd_i->_vs_code.size()] = 0;
  strcpy(fs_code, sd_shd_i->_fs_code.c_str());
  fs_code[sd_shd_i->_fs_code.size()] = 0;
}
void edit_item(shader::sd_shader_ide &sd_shd_i) {
  if (ImGui::Button("Reload")) {
    string vs_shd_path =
        g_cureent_directory + shaders_fold + sd_shd_i->_vs_name;
    auto &_vs_code = sd_shd_i->_vs_code;
    ...
    string fs_shd_path =
        g_cureent_directory + shaders_fold + sd_shd_i->_fs_name;
    auto &_fs_code = sd_shd_i->_fs_code;
  ...
  if (ImGui::Button("Compile")) {
    sd_shd_i->build_vs_code(vs_code);
    sd_shd_i->build_fs_code(fs_code);
    if (sd_shd_i->link()) {
      sd_shd_i->_vs_code = vs_code;
      sd_shd_i->_fs_code = fs_code;
    }
  }
