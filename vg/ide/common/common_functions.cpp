...
y'=-(x-a)sin��+(y-b)cos��+b
*/
void saveTextureToFile(GLuint textureID, const char *filename) {
  ...
  unsigned char *pdata =
      new unsigned char[width * height * channels]; // RGBA格式
  glGetTexImage(GL_TEXTURE_2D, 0, internalFormat, GL_UNSIGNED_BYTE, pdata);
  string str_filename(filename);
  string ex_type = str_filename.substr(str_filename.length() - 3, 3);
  map<string, int> ex_type_2_save_type = {
      {"png", SOIL_SAVE_TYPE_PNG},
      {"jpg", SOIL_SAVE_TYPE_JPG},
      {"bmp", SOIL_SAVE_TYPE_BMP},
      {"tga", SOIL_SAVE_TYPE_TGA},
      {"dds", SOIL_SAVE_TYPE_DDS},
  };
  auto save_type = ex_type_2_save_type[ex_type];
  if (SOIL_save_image(filename, save_type, width, height,
                      channels,
                      pdata) ==
      0) {
...
bool prepareFBO_ex(GLuint &texture_id, GLuint &FBO_id, GLuint &RBO_id,
                   GLuint frame_width, GLuint frame_height) {
...
void rescale_framebufffer(GLuint &texture_id, GLuint &RBO_id,
                          GLuint frame_width, GLuint frame_height) {
  glBindTexture(GL_TEXTURE_2D, texture_id);
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, frame_width, frame_height, 0, GL_RGB,
               GL_UNSIGNED_BYTE, NULL);
...
GLuint FBO{0}, texture_id{0}, RBO{0}, WIDTH{500}, HEIGHT{500};
...
void store_to_clipboard( string& str_content )
{
  if (!str_content.empty() && OpenClipboard(NULL)) {
    HGLOBAL wbuf_handle =
        GlobalAlloc(GMEM_MOVEABLE, (SIZE_T)str_content.size() + 1);
    if (wbuf_handle) {
