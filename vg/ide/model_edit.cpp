...
void processMesh(aiMesh *mesh, const aiScene *scene, vertex_object& obj_pm, vg_mesh& mesh_unit)
{
	...
	vg_vertex* pvertexs = new vg_vertex[mesh->mNumVertices];
	...
	auto float_size = sizeof(vg_vertex) / sizeof(float);
	...
	ps_vg_file ps_file = make_shared<vg_file>(buff_len);
	...
	*phead_len = float_cnt*sizeof(float);
	...
	delete[] pvertexs;
	delete[] pface_idx;
	g_dic_file_chunk[mesh_unit._prm_id] = ps_file;
...
void processNode(aiNode *node, const aiScene *scene, vg_model& md, string& mesh_base_name)
{
