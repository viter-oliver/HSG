...
common_value_edit g_common_value_edit;
...
void drag_dop_callback(GLFWwindow *wh, int cnt, const char **fpaths)
{
     ...
     _ui_as.load_afg_from_file(g_cureent_project_file_path.c_str()); 
               ...
               if (backup_list.size() == 0)
               {
                    ...
                    for (afg_fs::directory_iterator dir_itr(prj_path); dir_itr != end_iter; ++dir_itr)
                    {
                         try
                         {
                              ...
                              {
                                   file_chunk_unit file_unit = {dir_itr->path().filename().string(), afg_fs::last_write_time(dir_itr->path())};
                    ...
                    sort(backup_list.begin(), backup_list.end(), [](file_chunk_unit &fu0, file_chunk_unit &fu1)
