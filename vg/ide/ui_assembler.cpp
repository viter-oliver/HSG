...
{
     auto vres_texture_constrain = [&]()
     { return g_vpacking_texture.size() > 0; };
     factory::get().register_res_constrained("ctl_image", vres_texture_constrain);
     factory::get().register_res_constrained("ctl_button", vres_texture_constrain);
     factory::get().register_res_constrained("ctl_polygon_image", vres_texture_constrain);
     factory::get().register_res_constrained("ctl_slider", vres_texture_constrain);
     factory::get().register_res_constrained("ctl_slider_thumb", vres_texture_constrain);
     factory::get().register_res_constrained("ctl_quad_image", vres_texture_constrain);

     factory::get().register_res_constrained("ctl_modeling_3d", [&]()
...
void clear_pre_proj_resource()
{
     ...
     g_dic_indep_texture.clear();
     g_dic_file_chunk.clear();
     g_material_list.clear();
     g_vg_shader_list.clear();
...
bool ui_assembler::load_vg_from_file(const char *file_path)
{
     ...
     if (fin.is_open())
     {
          ...
          if (reader.parse(fin, jroot, false))
          {
               ...
               if (!screenw.isNull())
               {
                    control_common_def::screenw = screenw.asDouble();
               }
               else
               {
                    control_common_def::screenw = 2000.0f;
               }
               if (!screenh.isNull())
               {
                    control_common_def::screenh = screenh.asDouble();
               }
               else
               {
                    control_common_def::screenh = 1000.0f;
               }
               ...
               if (!fonts.isNull())
               {
                    ...
                    for (int ix = 0; ix < isz; ix++)
                    {
                         ...
                         if (ctl_u)
                         {
                              ...
                              ctl_u->_char_count_c = jfont["cols"].asInt();
                              ctl_u->_char_count_r = jfont["rows"].asInt();
                         }
                    }
               }
               ...
               texture_res_load tresload(g_vpacking_texture);
               ...
               for (ix = 0; ix < isize; ix++)
               {
                    ...
                    auto pimge = make_shared<vg_texture>();
                    ...
                    g_dic_indep_texture[kname] = pimge;
               }
               ...
               for (ix = 0; ix < fsize; ix++)
               {
                    ...
                    ifs.seekg(0, ios::beg);
                    g_dic_file_chunk[kname] = make_shared<vg_file>(res_size);
                    ifs.read((char *)g_dic_file_chunk[kname]->_pbin, res_size);
                    ifs.close();
               }
               ...
               for (ix = 0; ix < isize; ix++)
               {
                    ...
                    ifs_shd.close();
                    shared_ptr<vg_shader> pshd = make_shared<vg_shader>(vs_code.c_str(), fs_code.c_str());
                    pshd->_vs_name = shd_unit["vs_name"].asString();
                    pshd->_fs_name = shd_unit["fs_name"].asString();
                    pshd->set_name(shd_unit["name"].asString());
                    g_vg_shader_list[shd_unit["name"].asString()] = pshd;
               }
               ...
               if (models.isObject())
               {
                    ...
                    for (auto &md_nm : md_mb)
                    {
                         ...
                         auto pmodel = make_shared<vg_model>();
                         ...
                         for (int iix = 0; iix < iisz; ++iix)
                         {
                              ...
                              for (int jd = 0; jd < jdsize; ++jd)
                              {
                                   ...
                                   auto &idiff = g_dic_indep_texture.find(diff_txt);
                                   if (idiff != g_dic_indep_texture.end())
                                   {
                              ...
                              for (int jd = 0; jd < jdsize; ++jd)
                              {
                                   ...
                                   auto &ispec = g_dic_indep_texture.find(specular_txt);
                                   if (ispec != g_dic_indep_texture.end())
                                   {
                              ...
                              for (int jd = 0; jd < jdsize; ++jd)
                              {
                                   ...
                                   auto &iheight = g_dic_indep_texture.find(height_txt);
                                   if (iheight != g_dic_indep_texture.end())
                                   {
                              ...
                              for (int jd = 0; jd < jdsize; ++jd)
                              {
                                   ...
                                   auto &iamb = g_dic_indep_texture.find(ambient_txt);
                                   if (iamb != g_dic_indep_texture.end())
                                   {
               ...
               auto jarry_2_prp_pos = [this](Value &jarry, prop_ele_position &prp_epos)
               {
                    ...
                    control_common_def *pcontrol = &_root;
                    int ii = con_id_max;
                    for (; ii > 1; ii--)
                    {
                         Value &jcid = jarry[ii];
               ...
               for (auto item = memb.begin(); item != memb.end(); ++item)
               {
                    ...
                    auto ps_stm = make_shared<vg_state_manager>();
                    auto &stm = *ps_stm;
               ...
               for (int ix = 0; ix < ifbsz; ix++)
               {
                    ...
                    g_feedback_list[fkey] = make_shared<vg_feedback>(imtl->second, iprm->second);
               }
...
bool ui_assembler::output_ui_component_to_file(const char *file_path)
{
     ...
     try
     {
          jroot["screenw"] = control_common_def::screenw;
          jroot["screenh"] = control_common_def::screenh;
          ...
          for (auto &reslist_item : g_vpacking_texture)
          {
          ...
          for (auto &txt : g_dic_indep_texture)
          {
...
void update_ui_component_texture_res_index(control_common_def &tar, packing_txt_cd &old_txt_cd, packing_txt_cd &new_txt_cd, int gp_idx);
bool ui_assembler::load_ui_component_from_file(control_common_def &insert_node, char *file_path)
{
     HCURSOR hcur_cursor = GetCursor();
     SetCursor(g_hcursor_wait);
     ifstream fin;
     ...
     fin.open(file_path);
     locale::global(locale("C"));
     string tar_vg_path(file_path);
     string tar_vg_dir_path = tar_vg_path.substr(0, tar_vg_path.find_last_of('\\') + 1);
     if (fin.is_open())
     {
          Reader reader;
          Value jroot;
          if (reader.parse(fin, jroot, false))
          {
               ...
               auto find_text_group = [](v_sd_packing_texture &res_container, string &pack_file)
               {
               ...
               int group_sz = g_vpacking_texture.size();
               ...
               for (int ix = 0; ix < tsize; ++ix)
               {
                    ...
                    auto gp_id = find_text_group(g_vpacking_texture, texture_pack_file.asString());
                    str_cmd = "copy ";
                    str_cmd += "\"";
                    ...
                    {
                         packing_texture &cur_txt_list = *g_vpacking_texture[gp_id];
                         packing_txt_cd old_txt_cd = cur_txt_list.vtexture_coordinates;
                         ...
                         packing_txt_cd &new_txt_cd = cur_txt_list.vtexture_coordinates;
                         int chldcnt = _root.child_count();
                         for (size_t ix = 0; ix < chldcnt; ++ix)
                         {
                              control_common_def *pchild = _root.get_child(ix);
                              update_ui_component_texture_res_index(*pchild, old_txt_cd, new_txt_cd, gp_id);
                         }
                    }
                    else
                    {
                         g_vpacking_texture.emplace_back(make_shared<packing_texture>());
                         packing_texture &rtlist = *g_vpacking_texture[gp_id];
               ...
               for (ix = 0; ix < isize; ix++)
               {
                    ...
                    system(str_cmd.c_str());
                    ps_vg_texture pimge;
                    auto ifd = g_dic_indep_texture.find(kname);
                    if (ifd == g_dic_indep_texture.end())
                    {
                         pimge = make_shared<vg_texture>();
                         g_dic_indep_texture[kname] = pimge;
                    }
                    else
                    {
               ...
               for (ix = 0; ix < fsize; ix++)
               {
                    ...
                    ifs.seekg(0, ios::beg);
                    ps_vg_file pfile;
                    auto ifd = g_dic_file_chunk.find(kname);
                    if (ifd == g_dic_file_chunk.end())
                    {
                         pfile = make_shared<vg_file>(res_size);
                         g_dic_file_chunk[kname] = pfile;
                    }
                    else
                    {
                         pfile = ifd->second;
               ...
               for (ix = 0; ix < isize; ix++)
               {
                    ...
                    string vg_vs = str_cmd + vs_code_f;
                    vg_vs += "\" \"";
                    vg_vs += str_cur_shader_path;
                    vg_vs += "\"";
                    system(vg_vs.c_str());
                    string vg_fs = str_cmd + fs_code_f;
                    vg_fs += "\" \"";
                    vg_fs += str_cur_shader_path;
                    vg_fs += "\"";
                    system(vg_fs.c_str());
                    auto sd_name = shd_unit["name"].asString();
                    ps_shader pshd;
                    auto ifd = g_vg_shader_list.find(sd_name);
                    if (ifd != g_vg_shader_list.end()) // maybe result in some link error when constructing new controls
                    {
                         pshd = ifd->second;
                    ...
                    {
                         pshd = make_shared<vg_shader>(vs_code.c_str(), fs_code.c_str());
                         g_vg_shader_list[sd_name] = pshd;
                    }
               ...
               for (ix = 0; ix < isize; ix++)
               {
                    ...
                    if (pmtl->init_from_json(mtl_unit))
                    {
                         auto ifd = g_dic_file_chunk.find(pmtl->get_name());
                         if (ifd != g_dic_file_chunk.end())
                         {
                              g_dic_file_chunk.erase(ifd);
                         }
                         g_material_list[pmtl->get_name()] = pmtl;
                    }
               }
               ...
               auto jarry_2_prp_pos = [&](Value &jarry, prop_ele_position &prp_epos)
               {
                    ...
                    assert(jsize > 1);
                    int con_id_max = jsize - 1;
                    control_common_def *pcontrol = &insert_node;
                    int ii = con_id_max;
                    for (; ii > 1; ii--)
                    {
                         Value &jcid = jarry[ii];
...
int real_id_after_update(string &file_name, packing_txt_cd &tar_txt_cd)
{
...
void update_ui_component_texture_res_index(control_common_def &tar, packing_txt_cd &old_txt_cd, packing_txt_cd &new_txt_cd, int gp_idx)
{
     ...
     for (size_t ix = 0; ix < chldcnt; ++ix)
     {
          control_common_def *pchild = tar.get_child(ix);
...
bool ui_assembler::update_texture_res()
{
     ...
     int isize = g_vpacking_texture.size();
     for (int idx = 0; idx < isize; ++idx)
     {
          packing_texture &cur_txt_list = *g_vpacking_texture[idx];
          packing_txt_cd old_txt_cd = cur_txt_list.vtexture_coordinates;
          unsigned int txtid = cur_txt_list.texture_id();
          glDeleteTextures(1, &txtid);
          cur_txt_list.txt_id = 0;
...
ps_primrive_object ui_assembler::load_primitive_from_file(string &kname, vector<GLubyte> ele_format, GLuint vbo_len, GLuint ebo_len)
{
     GLfloat *pvbo = 0;
     GLuint *pebo = 0;
     auto ifile = g_dic_file_chunk.find(kname);
     if (ifile != g_dic_file_chunk.end())
     {
