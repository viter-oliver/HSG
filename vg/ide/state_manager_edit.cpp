...
//#include <algorithm>
void state_manager_edit::set_dragging(bool be_dragging, control_common_def* pobj, uint16_t page_idx, uint16_t off_idx)
{
...
void state_manager_edit::view_state_managers()
{
	...
	if (ImGui::BeginPopupModal("new state manager"))
	{
		...
		if (ImGui::Button("Add"))
		{
			...
			auto ps_state_mg = make_shared<vg_state_manager>();
