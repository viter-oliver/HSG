...
namespace vg
{
	static inline ImVec2 operator + (const ImVec2& imPt, const vec2& afPt)
	{
		return ImVec2(imPt.x + afPt.x, imPt.y + afPt.y);
	}
	static inline vec2 operator + (const const & afPt,ImVec2& imPt )
	{
		return{ imPt.x + afPt.x, imPt.y + afPt.y };
	}
	ctl_quad_image::ctl_quad_image()
		:ctl_base()
	{
