...
namespace vg
{
     ctl_modeling_3d::ctl_modeling_3d()
     {
         ...
         // _pty_page._trans = { { 0.15f, 0.15f, 0.15f }, { 0, 0, 0 }, { 0, 0, 0 }, true };
          ...
          auto shd_modeling = g_vg_shader_list.find( str_modeling );
          if( shd_modeling == g_vg_shader_list.end() )
          {
               _pshd_modeling = make_shared<vg_shader>( modeling_vs, modeling_fs );
               _pshd_modeling->set_name( str_modeling );
               _pshd_modeling->_vs_name = str_modeling + ".vs";
               _pshd_modeling->_fs_name = str_modeling + ".fs";
               g_vg_shader_list[ modeling ] = _pshd_modeling;
          }
          else
          {
...
#define PI 3.1415926545
          ...
          reg_property_handle( &_pty_page, 10, [this]( void* member_address )
          {
               ...
               else
               {
                    ...
                    if( ImGui::Button( "Import" ) )
                    {
                         auto itxt = g_dic_indep_texture.find( _pty_page._txt_diffuse );
          ...
          reg_property_handle( &_pty_page, 11, [this]( void* member_address )
          {
               ...
               else
               {
                    ...
                    if( ImGui::Button( "Import" ) )
                    {
                         auto itxt = g_dic_indep_texture.find( _pty_page._txt_specular );
                         if( itxt != g_dic_indep_texture.end() )
                         {
     ...
     {}
#if defined(WITHIN_IDE)
     void ctl_modeling_3d::view_components_in_world_space()
     {
          ...
          ImVec2 lt_pos = pos0;
          vec3  ltPos { _pty_page._light_posx, _pty_page._light_posy, _pty_page._light_posz };
          switch( _dir_view )
          {
               case 0:
          ...
          vec3& lt_spcol = _pty_page._light_specular_clr;
     ...
     void ctl_modeling_3d::link()
     {
          ...
          itxt = g_dic_indep_texture.find( _pty_page._txt_specular );
          if( itxt != g_dic_indep_texture.end() )
          {
               _pspecular = itxt->second;
          }
          string str_modeling( modeling );
          auto shd_modeling = g_vg_shader_list.find( modeling );
          if( shd_modeling == g_vg_shader_list.end() )
          {
               _pshd_modeling = make_shared<vg_shader>( modeling_vs, modeling_fs );
               _pshd_modeling->set_name( str_modeling );
	...
	void ctl_modeling_3d::draw()
	{
		if (!_pmodel)
		{
			return;
		}
		vg_model& my_model = *_pmodel;
		vg_shader& my_shader =*_pshd_modeling;
		my_shader.use();
		glm::mat4 model;
          for( auto& item : _vchilds )
          {
               ctl_trans* ptrans_item = (ctl_trans*)item;
