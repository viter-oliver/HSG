...
namespace vg
{
	...
	{
		auto& nm_list = g_pfont_face_manager->get_dic_fonts();
		...
		: _txt_area(0.f, 0.f, 0.f, 0.f)
	{
		...
		reg_property_handle(&_txt_pt, 9, [this](void*){
			...
			if (ImGui::Combo("font:", &_txt_pt._font_id, &get_font_item, 0, nm_list.size()))
			{
...
#endif
     bool textblock::contains(float posx, float posy)
	{
		ImVec2 mouse_pos(posx, posy);
		bool be_contain = _txt_area.Contains(mouse_pos);
		return be_contain;
	}
	bool textblock::relative_contain(
	{
		ImVec2 abpos = absolute_coordinate_of_base_pos();
		ImVec2 winpos = ImGui::GetWindowPos();
	...
	bool textblock::relative_contain(float pos, bool be_h)
	{
