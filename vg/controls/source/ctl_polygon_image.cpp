...
namespace vg
{
	...
	void ctl_polygon_image::read_track_file(const char* track_file)
	{
		auto& ifile = g_dic_file_chunk.find(track_file);
	...
	void ctl_polygon_image::link()
	{
		read_track_file(_img_pt._track_file);
	}
	ctl_polygon_image::ctl_polygon_image()
		:ctl_base()
	{
	...
	void ctl_polygon_image::draw()
	{
		if (!_ps_track_file)
		{
          ...
          auto& cur_res_list = *g_vpacking_texture[ img_id.x ];
