...
namespace vg
{
	...
	ctl_plane_play_2d::ctl_plane_play_2d()
		:ctl_base(), _puv(0), _uv_len(0)
	{
		...
		reg_property_handle(&_pt, [this](void*){
			...
			{
				auto atxt = g_dic_indep_texture.find(_pt._texture_name);
				if (atxt != g_dic_indep_texture.end())
				{
			...
			{
				auto atxt_fmt = g_dic_file_chunk.find(_pt._texture_fmt_name);
				if (atxt_fmt != g_dic_file_chunk.end())
				{
	...
	void ctl_plane_play_2d::link()
	{
		...
		auto atex = g_dic_indep_texture.find(_pt._texture_name);
		if (atex!=g_dic_indep_texture.end())
		{
			_texture = atex->second;
		}
		auto amuf = g_dic_file_chunk.find(_pt._texture_fmt_name);
		if (amuf!=g_dic_file_chunk.end())
		{
			_puv = get_txt_uvs((char*)amuf->second->_pbin, _uv_len);
		}
	}

	void ctl_plane_play_2d::draw()
	{
