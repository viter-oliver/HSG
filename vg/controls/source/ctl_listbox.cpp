...
namespace vg
{
		...
		, max_display_num(-1)
	{
		...
		reg_property_handle(&_in_p, 2, [this](void*){
			...
			if (ImGui::SliderFloat("Width", &_in_p._sizew, 0.f, control_common_def::screenw))
			{
		...
		reg_property_handle(&_in_p, 3, [this](void*){
			...
			if (ImGui::SliderFloat("Height", &_in_p._sizeh, 0, control_common_def::screenh))
			{
				update_draw_list_pos();
			}
		});
	...
	void ctl_listbox::draw()
	{
		if (max_display_num == -1)
		{
          ...
          auto& cur_res_list = *g_vpacking_texture[ img_txt_id.x ];
          int texture_id = cur_res_list.texture_id();
          packing_txt_cd& ptext_cd = cur_res_list.vtexture_coordinates;
		if (ptext_cd.size() == 0)
			return;
	...
	void ctl_listbox::draw_frames()
	{
		draw();
	}

	void ctl_listbox::update_draw_list_pos()
	{
		if (_vchilds.empty())
		{
	...
	void ctl_listbox::update_highlight_pos()
	{

	}

	void ctl_listbox::add_child(control_common_def* pchild)
	{
		ctl_base* _pnode = new ctl_base;

		ctl_base::add_child(_pnode);
	...
	void ctl_listbox::remove_child(control_common_def* pchild)
	{
		for (auto item = _vchilds.begin(); item != _vchilds.end(); item++)
		{
			if ((*item)->get_child(0) == pchild)
			{
				control_common_def* _pchild = *item;
				_vchilds.erase(item);
				delete _pchild;
				break;
			}
	...
	void ctl_listbox::set_cur_index(int index)
	{
	...
	void ctl_listbox::set_max_display_num(int num)
	{
