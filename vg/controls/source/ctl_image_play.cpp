...
namespace vg
{
		...
		:ctl_base()
	{
		...
		reg_property_handle(&_img_pt, 0, [this](void*){
			...
			else
			{
				...
				{
					auto atex = g_dic_indep_texture.find(_img_pt._texture_name);
		...
		reg_property_handle(&_img_pt, 3, [this](void*){
			...
			else
			{
				...
				{
					auto atxt_fmt = g_dic_file_chunk.find(_img_pt._texture_fmt_name);
	...
	void ctl_image_play::draw()
	{
		...
		//ctl_base::draw();
		packing_txt_cd& ptext_cd = _vtexture_cd;
