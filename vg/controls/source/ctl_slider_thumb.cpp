...
namespace vg
{
	ctl_slider_thumb::ctl_slider_thumb()
	{
		...
		reg_property_handle(&_img_pt, 5, [this](void*){
			...
			if (ImGui::SliderFloat("min width", &_img_pt._size_minw, 1.0, control_common_def::screenw) && _keep_min_scale&&_w2h_min > 0.001)
			{
				_img_pt._size_minh = _img_pt._size_minw / _w2h_min;
			}
		});
		reg_property_handle(&_img_pt, 6, [this](void*){
			if (ImGui::SliderFloat("min height", &_img_pt._size_minh, 1.0, control_common_def::screenh) && _keep_min_scale)
			{
		...
		reg_property_handle(&_img_pt,9, [this](void*){
			...
			if (ImGui::SliderFloat("max width", &_img_pt._size_maxw, 1.0, control_common_def::screenw) && _keep_max_scale&&_w2h_max > 0.001)
			{
				_img_pt._size_maxh = _img_pt._size_maxw / _w2h_max;
			}
		});
		reg_property_handle(&_img_pt, 10, [this](void*){
			if (ImGui::SliderFloat("max height", &_img_pt._size_maxh, 1.0, control_common_def::screenh) && _keep_max_scale)
			{
	...
	void ctl_slider_thumb::calcu_thumb_pos()
	{
