...
namespace vg
{
     ...
     void ctl_essay::load_content( wstring& str_content )
     {
          if( !_pfont_unit )
          {
               return;
          ...
          for (auto& ch:str_content)
          {
               ...
               {
                    wsparagraph += ch;
                    auto pnew_sentence = new ctl_sentence( _pfont_unit, wsparagraph );
     ...
     void ctl_essay::draw()
     {
          ...
          auto r_edge = wsz.x - hmargin+winpos.x;
           draw_pos = {hmargin,vmargin};
           prev_end_pos;
          for (auto ich:_vchilds)
          {
               ctl_sentence* pstc = static_cast<ctl_sentence*>( ich );
          ...
          auto swap_ui = [&]( control_common_def* pui1, control_common_def*pui2 )
          {
          ...
          {
               static control_common_def* psel_ui = nullptr;
               ...
               {
                    control_common_def* phit = get_hit_ui_object( io.MousePos.x, io.MousePos.y );
                    if (phit)
                    {
     ...
     bool ctl_essay::orignal_order()
     {
          int ix = 0;
          for (auto& itc:_vchilds)
          {
               ctl_sentence* pst = static_cast<ctl_sentence*>( itc );
               if( pst->load_idx !=ix)
               {
                    return false;
               }
