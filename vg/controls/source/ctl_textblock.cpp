...
namespace vg
{
	...
	static bool get_font_item(void* data, int idx, const char** out_str)
	{
		...
		*out_str = ctl_nm_list[idx]->_name.c_str();
		...
		: _txt_area(0.f, 0.f, 0.f, 0.f)
	{
		...
		reg_property_handle(&_txt_pt, 9, [this](void*){
			...
			if (ImGui::Combo("font:", &_txt_pt._font_id, &get_font_item, 0, ctl_nm_list.size()))
			{
				_pfont_unit = ctl_nm_list[_txt_pt._font_id];
			}
		});
	...
	void ctl_textblock::link()
	{
		auto& ctl_nm_list = g_pfont_face_manager->get_dic_fonts();
		if (_txt_pt._font_id>=ctl_nm_list.size())
		{
			_txt_pt._font_id = 0;
		}
		_pfont_unit = ctl_nm_list[_txt_pt._font_id];
	}
	void ctl_textblock::draw()
	{
		if (!_pfont_unit)
		{
			return;
		...
		auto& ctl_nm_list = g_pfont_face_manager->get_dic_fonts();
		auto font_cnt = ctl_nm_list.size();
		if (font_cnt==0)
		{
			return;
		}
		...
		//string font_name = ctl_nm_list[_txt_pt._font_id]->_name;
...
#if defined(WITHIN_IDE)
     void ctl_textblock::draw_outline()
     {
          ImVec2 abpos = absolute_coordinate_of_base_pos();
          ImVec2 winpos = ImGui::GetWindowPos();
          ImVec2 dpos = abpos + winpos;
...
#endif
     bool ctl_textblock::contains(float posx, float posy)
	{
		ImVec2 mouse_pos(posx, posy);
		bool be_contain = _txt_area.Contains(mouse_pos);
		return be_contain;
	}
	bool ctl_textblock::relative_contain(
	{
		ImVec2 abpos = absolute_coordinate_of_base_pos();
		ImVec2 winpos = ImGui::GetWindowPos();
	...
	bool ctl_textblock::relative_contain(float pos, bool be_h)
	{
