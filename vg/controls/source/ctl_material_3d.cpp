...
ctl_material_3d::ctl_material_3d() {
  ...
  reg_property_handle(&_pt, 8, [this](void *memb_adress) {
    ImGui::Combo("trans order:", &_pt._trans_order, str_trans_order,
                 en_trans_order_cnt);
  });
...
bool ctl_material_3d::get_output_vertex(vector<float> &overtex) {
  if (!_pt._with_feedback) {
    return false;
  }
  overtex.resize(_ps_prm->_vertex_buf_len);
  glGetBufferSubData(GL_TRANSFORM_FEEDBACK_BUFFER, 0,
                     _ps_prm->_vertex_buf_len * sizeof(GLfloat), &overtex[0]);
  return true;
}
void ctl_material_3d::draw() {
#if 1 // defined(WITHIN_IDE)
  ...
  if (_matched) {
    ...
    function<void()> f_rotate[en_rotate_order_cnt] = {
        [&]() {
          model = glm::rotate(model, _pt._trans_rotation_x * glm::radians(1.f),
                              glm::vec3(1.0f, 0.0f, 0.0f));
          model = glm::rotate(model, _pt._trans_rotation_y * glm::radians(1.f),
                              glm::vec3(0.0f, 1.0f, 0.0f));
          model = glm::rotate(model, _pt._trans_rotation_z * glm::radians(1.f),
