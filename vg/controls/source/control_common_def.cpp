...
namespace vg
{
		...
		reg_property_handle("camera", [](void *membadr)
							{
								...
								ImGui::Text("Camera");
								ImGui::SliderFloat("Position x", &pcamera->_position.x, -control_common_def::screenw, control_common_def::screenw);
								ImGui::SliderFloat("Position y", &pcamera->_position.y, -control_common_def::screenh, control_common_def::screenh);
								ImGui::SliderFloat("Position z", &pcamera->_position.z, -control_common_def::screenh, control_common_def::screenh);
		...
		reg_property_handle("point_light", [](void *membadr)
							{
								...
								ImGui::SliderFloat("Point light position x", &ppt_lt->_position.x, -control_common_def::screenw, control_common_def::screenw);
								ImGui::SliderFloat("Point light position y", &ppt_lt->_position.y, -control_common_def::screenh, control_common_def::screenh);
								ImGui::SliderFloat("Point light position z", &ppt_lt->_position.z, -control_common_def::screenh, control_common_def::screenh);
							});
		reg_property_handle("transformation", [](void *membadr)
							{
								...
								ImGui::SliderFloat("Translation x", &ptrans->_translation.x, -control_common_def::screenw, control_common_def::screenw, "%.3f", 0.1f);
								ImGui::SliderFloat("Translation y", &ptrans->_translation.y, -control_common_def::screenw, control_common_def::screenw, "%.3f", 0.1f);
								ImGui::SliderFloat("Translation z", &ptrans->_translation.z, -control_common_def::screenw, control_common_def::screenw, "%.3f", 0.1f);
							});
	...
	void control_common_def::draw_peropertys()
	{
		int pgidx = 0;
		...
		for (auto &prop_ele : _vprop_eles)
		{
			...
			else
			{
				...
				for (auto &memb : prop_page)
				{
					...
					bool be_base_type = mtype == "int" || mtype == "float" || mtype == "double" || mtype == "bool" || mtype == "vi2" || mtype == "vi3" || mtype =="vi4" || mtype == "vec2" || mtype =="vec3" || mtype =="vec4";
					...
					else
					{
						...
						else
						{
							...
							else if (mtype == "vi2")
							{
								if (rg == "txt") // atexture
								{
									f_draw_index_prop = [&](string &str_show, void *maddress)
									{
										vi2 *ptxt_idx = _vi2 *)maddress;
										int igsize = g_vpacking_texture.size();
										string str_gp = "Group id of" + str_show;
										ImGui::Combo(str_gp.c_str(), &ptxt_idx->x, get_texture_group_name, &g_vpacking_texture, igsize);
						...
						if (be_operating)
						{
							if (g.IO.MouseReleased[0])
							{
								...
								//print_buff( &bk_memb_value[ 0 ], bk_memb_value.size() );
								g_ui_edit_command_mg.create_command(edit_commd<control_common_def>(this, pmem_address, &bk_memb_value[0], bk_memb_value.size()));
								be_operating = false;
						...
						{
							g_ui_edit_command_mg.create_command(edit_commd<control_common_def>(this, memb_address, &before_op_memb_value[0], before_op_memb_value.size()));
	...
	void control_common_def::draw_outline()
	{
	...
	void control_common_def::init_property_from_json(Value &jvalue)
	{
		...
		for (size_t ix = 0; ix < chcnt; ix++)
		{
			...
			control_common_def *pcontrol_instance = factory::get().produce(cname);
			add_child(pcontrol_instance);
			pcontrol_instance->init_property_from_json(child);
		}
	}
	void control_common_def::init_property_from_json(Value &jvalue, dic_id &font_dic, dic_id &txt_dic)
	{
		for (auto &prop_ele : _vprop_eles)
		{
			auto &prop_page = prop_ele->_pro_page;
	...
	void control_common_def::save_property_to_json(Value &junit)
	{
		...
		for (size_t ix = 0; ix < chcnt; ix++)
		{
			control_common_def *pch_uc = get_child(ix);
