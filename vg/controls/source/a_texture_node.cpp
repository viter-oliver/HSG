...
namespace vg
{
	...
	using namespace packing_texture;
	 sd_shader_base a_texture_node::_ptxt_node_sd = nullptr;
     a_texture_node::a_texture_node()
     {
		 ...
		 if (!_ptxt_node_sd) {
			 _ptxt_node_sd = make_shared<shader_base>();// modeling_vs_1_txt, modeling_fs_1_txt);
			 _ptxt_node_sd->build_vs_code(modeling_vs_1_txt);
			 _ptxt_node_sd->build_fs_code(modeling_fs_1_txt);
			 _ptxt_node_sd->link();

		 }
     }
     void a_texture_node::transform( glm::mat4& model )
     {
		 glm::mat4 transM = glm::make_mat4(_pt._trans_mat4x4);
		 model = model*transM;
     }
	 void a_texture_node::link(){
	 ...
	 void a_texture_node::draw(){
		 ...
		 while (pd)
		 {
			 if (typeid(*pd) == typeid(a_texture_node)){
				 a_texture_node* pnode = static_cast <a_texture_node*>(pd);
		 ...
		 if (pscene == nullptr|| _pt._attached_txt>= _v_sd_packing_texture.size())
		 {
			 return;
		 }
		 ...
		 float aspect = pscene->size().x / pscene->size().y;
		 ...
		 auto& itxt = *_v_sd_packing_texture[_pt._attached_txt];
		 glBindTexture(GL_TEXTURE_2D, itxt.txt_id);
