...
void timer::execute() {
  auto currentTime = std_clock::now();
  for (auto eid = _will_erase_id_list.begin();
       eid != _will_erase_id_list.end();) {
    auto &tid = eid->first;
    auto &tdly = eid->second;

    int delta =
        std::chrono::duration_cast<std::chrono::duration<int, std::milli>>(
            currentTime - tdly._start)
            .count();
  ...
  for (auto eid = _will_erase_id_list0.begin();
       eid != _will_erase_id_list0.end();) {
    auto &tid = eid->first;
    auto &tdly = eid->second;
  ...
  for (auto &ictm : _active_tm_ex_list) {
    ...
    if (_timer_list_ex[ix]._handle && _timer_list_ex[ix]._tvalue) {
      int delta =
          std::chrono::duration_cast<std::chrono::duration<int, std::milli>>(
              currentTime - _timer_list_ex[ix]._tp)
              .count();
