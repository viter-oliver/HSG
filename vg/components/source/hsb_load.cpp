...
#include "playlist_group.h"
#define DXT5_DECOMPRESSED
#ifdef DXT5_DECOMPRESSED
#define GL_COMPRESSED_RGBA_S3TC_DXT5_EXT 0x83F3
#endif

#include "hsb_res_index.h"
using namespace vg;
void hsb_load::init_ui_component_by_mgo(control_base *&ptar, msgpack::v2::object &obj)
{
	...
	ptar = control_factory::instance().Create(str_type_name);
...
#endif
void hsb_load::load_afb(const char *afb_file)
{
...
#define TIME_CHECK(x)
	...
	unpac.next(oh);
	auto obj_w = oh.get();
	control_base::screenw = obj_w.via.array.ptr[en_screen_w].as<float>();
	control_base::screenh = obj_w.via.array.ptr[en_screen_h].as<float>();
	...
	for (size_t ix = 0; ix < font_cnt; ix++)
	{
		...
		auto ft_unit = font_manager::load_font(str_font_name, font_data_buff, data_sz);
	...
	for (size_t ix = 0; ix < re_cnt; ix++)
	{
		...
		packing_texture &res_unit = *g_vpacking_texture.back();
		...
		{
			res_unit.vtexture_coordinates.emplace_back(packing_texture_coordinate());
			packing_texture_coordinate &cd_unit = res_unit.vtexture_coordinates.back();
	...
	for (size_t ix = 0; ix < txt_cnt; ix++)
	{
		...
		memcpy(txt_kname, txt_name.via.str.ptr, txt_name_sz);
		g_dic_indep_texture[txt_kname] = make_shared<cp_texture>();
		auto &a_txt = g_dic_indep_texture[txt_kname];
	...
	for (size_t ix = 0; ix < file_cnt; ix++)
	{
		...
		auto a_file = make_shared<cp_file>(file_bin.via.bin.size);
		memcpy(a_file->_pbin, file_bin.via.bin.ptr, a_file->_fsize);
		g_dic_file_chunk[file_kname] = a_file;
		delete[] file_kname;
	}
	...
	if (g_output_bin_format._pgm_fmt == en_shader_code)
	{
		for (size_t ix = 0; ix < shd_cnt; ix++)
		{
			...
			g_cp_shader_list[pshd_name] = make_shared<cp_shader>(psd_vs_code, psd_fs_code);
			delete[] pshd_name;
			delete[] psd_vs_code;
			delete[] psd_fs_code;
		}
	...
	else if (g_output_bin_format._pgm_fmt == en_shader_bin_general)
	{
		for (size_t ix = 0; ix < shd_cnt; ix++)
		{
			...
			g_cp_shader_list[pshd_name] = make_shared<cp_shader>(binaryFormat, (void *)pbin, binCodeSize);
			delete[] pshd_name;
		}
	}
	...
	for (size_t ix = 0; ix < mtl_cnt; ix++)
	{
		...
		auto pshd = g_cp_shader_list.find(pstr_shd_name);
		if (pshd != g_cp_shader_list.end())
		{
	...
	for (size_t ix = 0; ix < obj_prm_size; ++ix)
	{
		...
		auto p_obj = make_shared<vertex>();
		auto ebo_len = pebo_len.as<GLuint>();
		auto vbo_len = p_vt_bf_len.as<GLuint>();
		GLfloat *pvbo = 0;
		GLuint *pebo = 0;
		auto ifile = g_dic_file_chunk.find(key_name);
	...
	for (size_t ix = 0; ix < obj_modles_sz; ++ix)
	{
		...
		for (size_t iy = 0; iy < obj_msh_lst_sz; iy++)
		{
			...
			for (size_t id = 0; id < obj_diffs_sz; ++id)
			{
				...
				const auto &idiff = g_dic_indep_texture.find(diff);
			...
			for (size_t id = 0; id < obj_speculars_sz; ++id)
			{
				...
				const auto &ispec = g_dic_indep_texture.find(specular);
			...
			for (size_t id = 0; id < obj_heights_sz; ++id)
			{
				...
				const auto &iheight = g_dic_indep_texture.find(height);
			...
			for (size_t id = 0; id < obj_ambients_sz; ++id)
			{
				...
				const auto &iamb = g_dic_indep_texture.find(ambient);
	...
	for (size_t ix = 0; ix < stm_sz; ix++)
	{
		...
		auto ps_stm = make_shared<cp_state_manager>();
		auto &stm = *ps_stm;
	...
	for (size_t ix = 0; ix < cmv_sz; ix++)
	{
		...
		auto ps_cmv = make_shared<base_common_type>(scmv_tp);
	...
	for (size_t ix = 0; ix < fdsz; ix++)
	{
		...
		g_feedback_list[fkey] = make_shared<cp_feedback>(imtl->second, iprm->second);
	}
