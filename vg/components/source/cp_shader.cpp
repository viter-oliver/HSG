...
{
	{ GL_FLOAT, { sizeof(float), 1 } }, { GL_FLOAT_VEC2, { sizeof(float), 2 } }, { GL_FLOAT_VEC3, { sizeof(float), 3 } }, { GL_FLOAT_VEC4, { sizeof(float), 4 } }, \
	{ GL_DOUBLE,{ sizeof(double), 1} }, { GL_DOUBLE_VEC2, { sizeof(double), 2 } }, { GL_DOUBLE_VEC3, { sizeof(double), 3 } },  { GL_DOUBLE_VEC4, { sizeof(double), 4 } }, \
	{ GL_INT, { sizeof(int), 1 }}, { GL_INT_VEC2, { sizeof(int), 2 } }, { GL_INT_VEC3, { sizeof(int), 3 } }, { GL_INT_VEC4, { sizeof(int), 4 } }, \
	{ GL_UNSIGNED_INT, { sizeof(unsigned int), 1 } }, { GL_UNSIGNED_INT_VEC2, { sizeof(unsigned int), 2 } }, { GL_UNSIGNED_INT_VEC3, { sizeof(unsigned int), 3 } }, { GL_UNSIGNED_INT_VEC4, { sizeof(unsigned int), 4 } }, \
...
cp_shader::cp_shader(GLenum binFormat, void* bin, GLsizei binLength)
{
...
cp_shader::cp_shader(const GLchar* vertex_shader_source, const GLchar* fragment_shader_source)
{
...
bool cp_shader::build_vs_code(string& vs_code)
{
	_vs_code = vs_code;
	char* pvs = &vs_code[0];
     if (_vertex_shader!=0)
...
bool cp_shader::build_fs_code(string& fs_code)
{
	_fs_code = fs_code;
	char* pfs = &fs_code[0];
     if( 0!=_frag_shader)
...
/**
void cp_shader::specify_transfeedback( bool relink )
{
...
*/
#if defined(WITHIN_IDE)
void cp_shader::refresh_sourcecode(string& vertex_shader_source, string& fragment_shader_source)
{
	if (is_valid())
	{
		glDeleteProgram(_shader_program_id);
...
void cp_shader::refresh_viarable_list()
{
	GLint idx;
	GLint count;
...
bool cp_shader::match_format(vector<GLubyte>& fmt)
{
...
bool cp_shader::vertex_att_pointer()
{
	int stride=0;
...
/*
template<typename T> bool cp_shader::uniform(string unf_name, GLsizei icnt, T* pvalue)
{
...
bool cp_shader::uniform(string unf_name, float* pvalue)
{
	auto tt = _unf_list.find(unf_name);
	if (tt == _unf_list.end())
	{
...
bool cp_shader::uniform(string unf_name, int* pvalue)
{
	auto tt = _unf_list.find(unf_name);
	if (tt == _unf_list.end())
	{
...
bool cp_shader::uniform(string unf_name, double* pvalue)
{
	auto tt = _unf_list.find(unf_name);
	if (tt == _unf_list.end())
	{
...
bool cp_shader::uniform(string unf_name, int ivalue)
{
	auto tt = _unf_list.find(unf_name);
	if (tt == _unf_list.end())
	{
