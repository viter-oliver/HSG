...
namespace vg {
    namespace state_sets {
        ...
        static const char* str_play_state[en_play_state_cnt] = { "stopped", "playing a trans",
                                                          "playing a playlist" };
        AFG_EXPORT bool trans_is_playing( state_set_key trans_name) {
            const auto& istm = state_set_map_.find(trans_name);
            if (istm == state_set_map_.end()) {
        ...
        AFG_EXPORT bool play_tran( state_set_key stm_name, int from, int to,
            bool cover_from_value) {
            const auto& istm = state_set_map_.find(stm_name);
            if (istm == state_set_map_.end()) {
                printf("invalid state manager name:%s\n", stm_name.value );
                return false;
            }
        ...
        AFG_EXPORT bool play_tran_playlist( state_set_key stm_name, int playlist_id) {
            const auto& istm = state_set_map_.find(stm_name);
            if (istm == state_set_map_.end()) {
                printf("invalid state manager name:%s\n", stm_name.value );
                return false;
        ...
        AFG_EXPORT bool restore_2_state( state_set_key trans_name, int state_id) {
            const auto& itrans = state_set_map_.find(trans_name);
            if (itrans == state_set_map_.end()) {
        ...
        AFG_EXPORT bool cancel_play_tran( state_set_key stm_name) {
            const auto& istm = state_set_map_.find(stm_name);
            if (istm == state_set_map_.end()) {
                printf("invalid state manager name:%s\n", stm_name.value );
                return false;
        ...
        AFG_EXPORT bool reg_trans_handle( state_set_key trans_name, trans_finish_handle trans_handle) {
            const auto& itrans = state_set_map_.find(trans_name);
            if (itrans == state_set_map_.end()) {
                return false;
            }
