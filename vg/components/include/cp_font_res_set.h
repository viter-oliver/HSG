	...
	*/
	...
	class font_face_manager
	{
		...
		void load_chars(FT_Face&fontFace, txt_font_repository&fp, wstring& wchar_list )
		{
			...
			vg::vui2& border = fp._border;
			vg::vi2& txt_size = fp._txt_size;
	...
	public:
		...
		ps_font_unit load_font(string& fontFaceName, string& fontPath,int& idx)
		{
          ...
          int draw_wstring( ps_font_unit& pf_u, GLint fontSize,
                              vg::vec2& start_pos, vg::vec2& end_pos,
                              GLfloat scale, wstring& str_content,
                              const vg::vec4& txt_col, float width,
                              int omit_rest, bool be_new );
          int draw_wstring( ps_font_unit& pf_u, GLint fontSize,
                            vg::vec2& start_pos, vg::vec2& end_pos,
                            GLfloat scale, wstring& str_content,
                            const vg::vec4& txt_col, vg::vec2&l_top_edge, 
                            vg::vec2&r_bottom_edge, float& line_spacing, bool be_new );
     };
	...
	bool _is_full{ false };
	public:
	vg_font_res_set(font_face_manager& font_mg);
	~vg_font_res_set();
	void draw_wstring(string& fontFace, GLuint fontSize, vg::vec2& start_pos, vg::vec2& end_pos,GLfloat scale, wstring& str_content,vg_vec3& txt_col,float width,bool omit_rest);
	};*/
