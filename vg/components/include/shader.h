...
namespace vg {
    ...
    namespace shader {
        ...
        using v_sd_shader_variable = std::vector<std::shared_ptr<sd_shader_variable>>;
        using mp_sd_shader_variable = std::map<std::string, sd_shader_variable>;
        class shader_variable_type :public uniform_interface {
        protected:
            u32 _location;
            u32 _ele_count;
        ...
        public:
            ...
            template <typename OT> void operator=(shader_variable_T<OT>& other) {
                if (typeid(OT) == typeid(T)) {
                    auto min_len = _buff_len > other.get_buff_len() ? other.get_buff_len() : _buff_len;
                    for (u32 ix = 0; ix < min_len; ++ix) {
                        _pdata[ix] = other[ix];
                    }
                }
...
#define REG_SHADER_VARIABLE_TYPE(enKey)    using sd_vtype_i##enKey=shader_variable_type_en< enKey >;  \
  static fac_shadre_variable_type::shader_type_assist<                         \
      ...
      reg##enKey(enKey)
        ...
        public:
            shader_variable_type_en(u32 loc, u32 elc) :shader_variable_type(loc, elc) {}
        ...
        using handle_uniform_pair = std::function<void(const std::string&, sd_shader_variable_type)>;
        class shader_base {
        protected:
            ...
            bool _valid_pm{ false }, _valid_vs{ false }, _valid_fs{ false };
        public:
            bool build_vs_code(const char* vertex_code);
            bool build_fs_code(const char* framgent_code);
            bool valid() { return _valid_vs&&_valid_fs&&_valid_pm; }
            ...
            void iterate_uniforms(handle_uniform_pair handle)
            {
                for (const auto& unf_u : _unf_list) {
                    handle(unf_u.first, unf_u.second);
                }
            }

            //gray@2024.09.11
            GLuint get_vertex_shader() { return _vertex_shader; }
            GLuint get_fragment_shader() { return _fragment_shader; }
            GLuint get_program_id() { return _program_id; }
            void set_vertex_shader(GLuint value) { _vertex_shader = value; }
            void set_fragment_shader(GLuint value) { _fragment_shader = value; }

            GLuint type2target(ShaderTargetType target_type);
            bool compile(ShaderTargetType target_type, std::string& error_info);
            bool link(std::string& error_info);
            void set_source(ShaderTargetType target_type, const std::string& source);
            bool get_source(ShaderTargetType target_type, std::string& source);
            bool check_shader_error(ShaderTargetType target_type, ShaderCheckType type, std::string& error_info);

        };
        DEF_TYPE_DIC(shader_base)
    } // namespace shader
