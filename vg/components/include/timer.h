...
class AFG_EXPORT timer {
  ...
  std::map<int, int> _active_tm_list, _active_tm_ex_list;
  // vector<int> _will_erase_id_list;
  std::map<int, timer_del> _will_erase_id_list, _will_erase_id_list0;

public:
  timer();
  ~timer();
  int register_timer(std::function<void(int)> hdl, int num_cr = 0, int fr = 0) {
    for (int ix = 0; ix < max_timer_num; ix++) {
      if (!_timer_list[ix]._handle) {
        ...
        std::swap(_timer_list[ix]._handle, hdl);
        return ix;
      }
    }
    return -1;
  ...
  void deactive_time(int timer_id, int delay_tm = 0) {
    ...
    if (iwid == _will_erase_id_list0.end()) {
      timer_del tdl = {std_clock::now(), delay_tm};
  ...
  int register_timer_ex(std::function<void(int)> hdl) {
    for (int ix = 0; ix < max_timer_num; ix++) {
  ...
  bool active_timer_ex(int timer_id, int tvalue) {
    if (timer_id < max_timer_num && _timer_list_ex[timer_id]._handle) {
      _timer_list_ex[timer_id]._tp = std_clock::now();
  ...
  bool active_timer_with_attachment(int timer_id, int tvalue,
                                    int timer_id_attached) {
