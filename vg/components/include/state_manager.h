...
#include <chrono>
namespace vg {
using prop_value_block = std::string;
using vprop_block = std::vector<prop_value_block>;
using vvprop_block = std::vector<vprop_block>;
...
using ps_state_manager = std::shared_ptr<cp_state_manager>;
using mp_state_manager = std::map<std::string, ps_state_manager>;
extern AFG_EXPORT mp_state_manager g_mstate_manager;
AFG_EXPORT bool trans_is_playing(std::string trans_name);
AFG_EXPORT bool save_trans_value(std::string trans_name, int sid);
AFG_EXPORT bool restore_trans_value(std::string trans_name, int sid);
...
AFG_EXPORT bool save_property_to_trans_state(std::string trans_name,
...
template <class T>
bool set_trans_pair_state_list_delta(
    std::string trans_name, prop_ele_position &prp_pos,
    int base_id, // start state index
    T base_value, std::vector<paire_value<T>> &vpvalue) {
...
template <class T>
bool set_trans_sequential_state_list_delta(std::string trans_name,
                                           prop_ele_position &prp_pos,
                                           int target_id, // start state index
                                           T tar_value,
                                           std::vector<T> &vpvalue) {
  const auto &itrans = g_mstate_manager.find(trans_name);
  if (itrans == g_mstate_manager.end()) {
    printf("invalid trans name:%s\n", trans_name.c_str());
    return false;
