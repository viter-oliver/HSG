...
struct factory
{
	...
	inline control_common_def* produce(const std::string& key)
	{
    ...
    #if 0
	template<typename... Args>
	control_common_def* produce(const std::string& key, Args&&... args)
	{
		if (map_.find(key) == map_.end())
			throw std::invalid_argument("the message key is not exist!");
    ...
    #endif
	std::unique_ptr<control_common_def> produce_unique(const std::string& key)
	{
		return std::unique_ptr<control_common_def>(produce(key));
	}

	std::shared_ptr<control_common_def> produce_shared(const std::string& key)
	{
		return std::shared_ptr<control_common_def>(produce(key));
	}
	...
	void iterate_types(function<void(string,function<control_common_def*()>)> imp)
	{
		for (auto it:map_)
		{
			bool be_valid = true;
...
private:
	...
	std::map<std::string, std::function<control_common_def*()>> map_;
...
//std::map<std::string, std::function<control_common_def*()>> factory::map_;
