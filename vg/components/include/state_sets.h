...
// const float cal_step = 0.025;
    ...
    struct state_set {
        state_sets::v_sd_state_unit _vstate_list;
        state_sets::state_transition _any_to_any;
        state_sets::trans_finish_handle _trans_finish;
        state_sets::sp_st_trans _pcur_tran{ nullptr };
        state_sets::mp_trans _mtrans; //
        unsigned char _state_idx{ 0 };
        state_sets::moving_state _mstate{state_sets::en_state_pause};
        state_sets::trans_play_state _play_state{state_sets::en_play_stop};
        state_sets::st_clock_point _trans_start;
        int _cur_from{ 0 }, _cur_to{ 0 };
        int _cur_play_trans_id{ 0 }, _cur_playlist_id{ 0 };
        state_sets::vvtrans_key _playlist_list;
    };

    DEF_TYPE_DIC( state_set )
} // namespace vg